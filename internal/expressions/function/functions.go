...
// SemverDiff returns an expr.Option that provides a `semverDiff()` function
// for use in expressions.
//
// The semverDiff function compares two semantic version strings and returns
// the difference level between them. It returns one of: "MAJOR", "MINOR",
// "PATCH", "METADATA", "NONE", or "INCOMPARABLE" if either version is invalid.
